...
fn main() {
    ...
    // 2^7 is the most efficient binary search
	let temp_storage:String = guesses.to_string();
	println!("Total guesses: {temp_storage}");


	match guesses.cmp(&7) {
        Ordering::Less => println!("You took less than the a binary search!"),
        Ordering::Greater => println!("how r u so inefficient!"),
        Ordering::Equal => {
            println!("literally bad luck but issok");
        }
    }
    ...
    another_function(55,'g'); // Give params actual namespacec
